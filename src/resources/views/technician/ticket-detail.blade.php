            ...
            <div class="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
                ...
                <div class="p-5">
                    <form method="POST" action="{{ route('technician.ticket.update-status', $ticket) }}"
                        ...
                        {{-- Action buttons --}}
                        <div class="grid grid-cols-1 sm:grid-cols-{{ min(count($nextStatuses), 3) }} gap-3">
                            ...
                            <label class="cursor-pointer status-btn" x-on:click="selected='{{ $status }}'">
                                ...
                                <div class="relative flex flex-col items-center gap-2 p-4 rounded-2xl text-white text-center transition-all duration-200 border-2 border-transparent"
                                     style="{{ $ac['style'] }}"
                                     :class="selected === '{{ $status }}' ? 'ring-4 ring-offset-2 ring-indigo-200 opacity-100 -translate-y-0.5 shadow-lg' : 'opacity-90 hover:opacity-100'">
                                    <svg class="w-6 h-6 opacity-90 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">{!! $ac['svg'] !!}</svg>
                                    <span class="text-xs font-bold leading-tight">{{ $ac['label'] }}</span>
                                    <div class="absolute -top-1.5 -right-1.5 w-[18px] h-[18px] bg-white rounded-full flex items-center justify-center shadow transition-all duration-200"
